...
const BubbleAssetCard: React.FC<{ asset: any }> = ({ asset }) => {
    return (
        <div className="glass-card border-l-4 border-l-rose-500 rounded-xl p-5 md:p-6 relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
            <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                <Flame className="w-12 h-12 md:w-16 md:h-16 text-rose-500" />
            </div>
            
            <div className="flex justify-between items-start mb-4 relative z-10">
                <div>
                    <h4 className="font-extrabold text-lg md:text-xl text-white tracking-tight">{asset.name}</h4>
                    <span className="text-[10px] text-slate-400 font-mono uppercase tracking-widest">{asset.sector}</span>
                </div>
                <div className={`px-2 py-1 md:px-3 md:py-1 rounded-lg font-bold text-[10px] md:text-xs uppercase tracking-wider ${asset.riskScore > 80 ? 'bg-rose-500 text-white shadow-lg shadow-rose-900/50' : 'bg-orange-500 text-slate-900'}`}>
                    {asset.riskScore}/100 Risk
                </div>
            </div>

            <div className="mb-4 relative z-10">
                <div className="text-2xl md:text-3xl font-mono text-rose-300 font-black">{asset.price}</div>
                <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Current Valuation</div>
            </div>

            <p className="text-xs md:text-sm text-slate-300 border-t border-white/5 pt-3 relative z-10 leading-relaxed font-medium">
                {asset.reason}
            </p>
        </div>
    );
  ...
  if (!data) {
     return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fade-in px-4">
            <div className="bg-rose-500/10 p-5 md:p-6 rounded-full mb-6 ring-1 ring-rose-500/50 shadow-[0_0_30px_rgba(244,63,94,0.2)]">
               <Activity className="w-10 h-10 md:w-12 md:h-12 text-rose-500" />
            </div>
            <h1 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tight uppercase">Market Bubble Scope</h1>
            ...
            <button 
               onClick={onScan} 
               disabled={isLoading}
               className="group relative flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 bg-rose-600 hover:bg-rose-500 text-white rounded-xl font-black uppercase tracking-widest transition-all shadow-[0_10px_40px_-10px_rgba(244,63,94,0.5)] hover:shadow-[0_20px_60px_-15px_rgba(244,63,94,0.6)] hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed text-xs md:text-sm"
            >
               {isLoading ? <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin" /> : <Play className="w-4 h-4 md:w-5 md:h-5 fill-current" />}
  ...
  return (
    <div className="animate-fade-in pb-20">
        ...
        <div className="relative mb-8 md:mb-12 text-center">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 md:px-4 md:py-1.5 rounded-full border border-rose-500/30 bg-rose-950/20 text-rose-300 text-[10px] font-extrabold mb-4 md:mb-6 uppercase tracking-widest shadow-[0_0_20px_rgba(244,63,94,0.15)]">
                <Activity className="w-3 h-3 animate-pulse" />
                Systemic Risk Monitor
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter mb-4 drop-shadow-2xl">Bubble Scope</h1>
        ...
        {isStreaming ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="glass-card p-5 md:p-6 rounded-2xl flex items-center justify-between">
                    <div>
                        <div className="text-slate-400 text-[10px] font-extrabold uppercase tracking-widest mb-2">Global Fragility</div>
                        <div className={`text-4xl md:text-5xl font-black ${structuredData.riskScore > 70 ? 'text-rose-500' : 'text-yellow-500'}`}>
                ...
                <div className="glass-card p-5 md:p-8 rounded-2xl md:col-span-2 flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap gap-2 md:gap-3">
                        {structuredData.warningSignals?.map((signal, i) => (
                            <span key={i} className="px-3 py-1.5 md:px-4 md:py-2 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-300 text-xs md:text-sm font-bold flex items-center gap-2 shadow-inner">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-8">
                <div>
                    <h3 className="text-xl md:text-2xl font-black text-white mb-6 flex items-center gap-3 uppercase tracking-tight">
                ...
                <div className="glass-card p-6 md:p-10 rounded-3xl">
                    <div className="flex items-center gap-3 mb-6 md:mb-8 border-b border-white/5 pb-6">
                        <Search className="w-5 h-5 md:w-6 md:h-6 text-sky-400" />
                        <h2 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight">Forensic Analysis</h2>
                        {isStreaming && <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin text-slate-500 ml-auto" />}
                    </div>
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                {/* Market Sentiment */}
                {structuredData && (
                     <div className={`p-5 md:p-6 rounded-2xl border backdrop-blur-md shadow-xl ${
                         ...
                         <div className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tight">{structuredData.marketSentiment || "Neutral"}</div>
                ...
                <div className="glass-card p-5 md:p-6 rounded-2xl">
                    <h3 className="text-[10px] font-extrabold text-white mb-4 uppercase tracking-widest flex items-center gap-2">
                        <ShieldAlert className="w-3 h-3 md:w-4 md:h-4 text-emerald-400" />
                        Safe Havens
                    </h3>
